...
class MultiBranchDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        multi_branch_data = {key: [] for key in data.keys()}
        for key in data.keys():
            for branch in data[key].keys():
                multi_branch_data[key] += list(filter(lambda x: x is not None, data[key][branch]))
        return self.data_preprocessor(multi_branch_data)
